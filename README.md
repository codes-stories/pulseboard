...
*   **goose:** For managing database migrations. Install it with:
    ```sh
    go install github.com/pressly/goose/v3/cmd/goose@latest
    ```
    Ensure Go's binaries are available in your shell. For zsh on macOS:
    ```sh
...
├── pluseboard-monitoring/ # Next.js frontend application.
...
4.  **Run Database Migrations:**
    ...
    make migrate-up
    ```
    If this is a new database and no migration files exist yet, first create one:
    ```sh
    make migrate-create name=initial_schema
    ```
    Add the schema's `CREATE TABLE` statements to the generated file in
    `backend/migrations/`, then run `make migrate-up` again. Check applied
    migrations at any time with `make migrate-status`.
...
| `make test` | Runs backend and agent tests plus frontend lint checks. |
...
| `make build-frontend` | Creates an optimized Next.js frontend build. |
| `make migrate-up` | Applies all pending database migrations. |
| `make migrate-down` | Rolls back the most recent database migration. |
| `make migrate-status` | Shows the status of all database migrations. |
| `make migrate-create name=...` | Creates a new SQL migration file. |
| `make clean` | Removes generated backend, agent, and frontend artifacts. |

---
*This README was generated by Gemini Code Assist.*

something which can't related ti this, just test pr policy
okk.
